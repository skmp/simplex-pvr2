...
//
// do_triangle_setup_pvr - the SAME setup as do_triangle_setup, line for line,
// but every value is an fpm<24> (float32 carried) and every arithmetic op runs at
// the PVR datapath's precision:
//   - multiplies use fp_mul<24,16> (fp_mul16: 16-bit input mantissa, 24-bit out)
//   - adds/subtracts use fpm<24> +/- (fp_add24)
//   - the area reciprocal is fp_rcp_fast (~16-bit) -> 1/area quantised to fpm<16>
// Structure is identical to the double version; only the precision differs.
//
                                    ...
                                    float xbase, float ybase)
{
    ...
    // ================= inputs as fpm<24> (float32 carried) =================
    ...
    bool    area_neg = (tri_area.tof32() < 0.0f);
    ...
    bool tl2 = ((DY23.tof32() == 0.0f && DX23.tof32() > 0.0f) || DY23.tof32() < 0.0f);
    bool tl3 = ((DY31.tof32() == 0.0f && DX31.tof32() > 0.0f) || DY31.tof32() < 0.0f);
...
// MAC product helper used by do_triangle_setup_pvr_existing below (fp_mul16:
// 16-bit input mantissa, 24-bit output).
static inline fpm<24> MUL(const fpm<24>& a, const fpm<24>& b) {
    return fp_mul<24, 16>(a, b);
}
