//
// timming_tsp_setup_min - standalone synthesis/timing harness that wraps the WHOLE
// tsp_setup_min unit. Sibling to timming_tsp_shade_v2_pp / stage_all: a self-contained
// place-and-route + timing-close context so the fitter reports Fmax/area for JUST the
// TSP setup core (its 4 mac16 lanes, reciprocal, and shared attr multipliers),
// decoupled from the rest of the pipeline.
//
// tsp_setup_min is a plain clk/reset unit (no DDR, no texture caches), so - like the
// per-stage stage_* harnesses - this uses a plain clocked top (no sysmem bridge).
//
// Pattern (identical to the other timing harnesses):
//   * ALL tsp_setup_min inputs are driven from a single free-running input register
...
//   * ALL tsp_setup_min outputs are captured into RAW registers with NO logic in
...
// give tsp_setup_min a self-contained fitting context for perf iteration.
//
// Build: cd timming_tests/timming_tsp_setup_min && quartus_sh --flow compile timming_tsp_setup_min
// Fmax:  output_files/timming_tsp_setup_min.sta.rpt
//
module timming_tsp_setup_min import tsp_pkg::*; (
    input             clk,
    input             reset,
    ...
    // ==================================================================
    // DUT: tsp_setup_min (the module under test).
    // ==================================================================
    wire        s_done;
    wire        s_plane_valid;
    ...
    // between - so the DUT-output -> register path is the PURE tsp_setup_min timing
