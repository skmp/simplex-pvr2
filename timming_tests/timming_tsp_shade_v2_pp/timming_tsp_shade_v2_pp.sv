//
// timming_tsp_shade_v2_pp (harness) - synthesis/timing harness wrapping the WHOLE
// tsp_shade_v2_pp pipeline. The v2 counterpart to stage_all (which wraps the old
// tsp_shade_pp).
//
// tsp_shade_v2_pp OWNS its texture caches (inside tex_unit) and exposes only:
//   * TWO DDR read ports (tc data, vq codebook) -> the real peel_core DDR read arbiter
//     (6 clients; only tc/vq live) -> the sysmem_lite Avalon read master.
//   * FOUR injected palette RAM read ports -> 4 harness-provided 1024x32 M10Ks.
// So unlike stage_all there are NO injected tc/vq caches here - just the palette RAMs.
//
// Pin count stays tiny: every DUT input is a slice of an HPS-poked input register bank
// (real register source per bit); every DUT output is captured raw then XOR-folded to a
// single `digest` pin (fitter can't prune any output path). Renders nothing - it exists
// only to place-and-route + timing-close tsp_shade_v2_pp in isolation.
//
module timming_tsp_shade_v2_pp import tsp_pkg::*; #(
    ...
    // ==================================================================
    // INPUT REGISTER BANK. 10 planes x 3 (ddx/ddy/c) = 30 words + scalars.
    //   0..9   : in_ddx[0..9]
    //   10..19 : in_ddy[0..9]
    //   20..29 : in_c  [0..9]
    //   30     : invw_in
    //   31     : tsp
    //   32     : tcw
    //   33     : { pp_offset[28], pp_texture[27], in_valid[26], text_ctrl[25:21],
    //             py[20:16], px[15:11], in_id[IDW-1:0] }
    //   34     : { pal_fmt[1:0] }
    // ==================================================================
    localparam integer NREG = 36;
    ...
    wire [31:0] w_c   [0:9];
    genvar gp;
    generate for (gp=0; gp<10; gp=gp+1) begin : unpack_planes
    ...
    wire [1:0]     w_palfmt = in_reg[34][1:0];
    ...
    // ==================================================================
    // OUTPUT CAPTURE: raw registers (pure DUT-output timing), then XOR-fold next cycle.
    // ==================================================================
    reg [31:0]    cap_argb, cap_tsp;
    reg [IDW-1:0] cap_id;
    reg           cap_valid, cap_stall;
    always @(posedge clk_100m) begin
        if (reset_100m) begin
            cap_argb<=32'd0; cap_tsp<=32'd0; cap_id<='0; cap_valid<=1'b0; cap_stall<=1'b0;
        end else begin
            cap_argb<=s_out_argb; cap_tsp<=s_out_tsp; cap_id<=s_out_id;
            cap_valid<=s_out_valid; cap_stall<=s_stall;
        end
    end
    always @(posedge clk_100m) begin
