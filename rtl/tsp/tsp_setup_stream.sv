...
//
//   * units: fp_mul16_spp_ro 2clk, fp_mul_c9_spp_ro 2clk, fp_add24_spp_ro 3clk
//     (align-to-max S1), fp_add3_24_spp_ro 4clk (split align), fp_rcp_faster 5clk.
//   * min-magnitude anchor: c = p_anch - ddx*XLa - ddy*YTa,
//     anchor = argmin_k max(|XLk|,|YTk|) (2-cycle tournament, cnt 8/9).
...
// triangles interleave safely in the delay lines (the 8-cycle guard is the static bound
...
//   serial     : done at cnt 28+2n   (n=10 -> 48)
//   back-to-back: sustained period 2n+14 clks/triangle (n=10 -> 34)
//
// -------------------------------------------------------------------------------
...
//   cnt 8/9   : min-magnitude anchor TOURNAMENT: cnt8 v1-vs-v2 -> {a12,aXL12,aYT12};
//               cnt9 winner-vs-v3 -> anch/aXLr/aYTr (valid cnt 10)
//   cnt 12    : rcp in (fp_rcp_faster, 5clk)               [rcy ready cnt 18]
//
...
//   +4  pa = anchor-mux(p1_r,p2_r,p3_r) -> pa_sr   (anchor valid cnt 10 <= T+4=12)
...
//   +11 MD: ddx = -Aa * rcy               (Aa on A2.y NOW; rcy ready cnt 18 <= T+11=19)
//   +12 MD: ddy = -Ba * rcy
...
//
// Latency: inject -> plane_valid = 21 clks. done pulses 1 clk after the last
...
module tsp_setup_stream (
    ...
    input             reset,
    input             start,
    output            rdy,       // 1: a new triangle can be latched THIS cycle
    output reg        done,
    ...
    // the delay lines after front clears; rdy re-asserts once the 8-cycle guard after
    // the LAST inject has elapsed (safe overlap bound - see header).
    reg        front;
    reg [3:0]  guard;
    reg [5:0]  cnt;
    reg [3:0]  nextp;
    reg        fin;
    assign rdy = !front && (guard == 4'd0);
    ...
    reg        last_sr [0:19];     // "last plane of its triangle" marker -> done
    ...
    // M0/M1: area products (cnt 6: X21 live on A0.y, Y31r/X31r/Y21r captured cnt 5)
    // then the plane mC stream (direct issue at T+5 / T+6)
    ...
    fp_mul16_spp_ro M0 (.clk(clk),.reset(reset),.stall(1'b0),
        .in_valid(front && cnt==6'd6 || d[5] || d[6]),
    ...
    // A3: c = pa - f0 - f1 (direct issue, 4-clock unit -> result at d[20])
    wire [31:0] a3_y;
    fp_add3_24_spp_ro A3 (.clk(clk),.reset(reset),.stall(1'b0),.in_valid(d[16]),
        .a(pa_sr[11]), .b(fneg(h_mf)), .c(fneg(mf_y)), .out_valid(), .y(a3_y));

    // RCP: 1/area, once per triangle (area = A0 result, valid cnt 12; 5-clock unit
    // -> y cnt 17, rcy valid cnt 18; first MD use at T+11 = 19)
    wire        rc_ack; wire [31:0] rc_y;
    fp_rcp_faster u_rcp (.clk(clk),.reset(reset),.stall(1'b0),
        .in_valid(front && cnt==6'd12), .x(a0_y), .out_valid(rc_ack), .y(rc_y));
    ...
    always @(posedge clk) begin
        if (reset) begin
            front<=1'b0; guard<=4'd0; done<=1'b0; plane_valid<=1'b0; fin<=1'b0;
            cnt<=6'd0; d<='0;
            g0v_r<=1'b0; g1v_r<=1'b0; g2v_r<=1'b0;
        end else begin
            done<=1'b0; plane_valid<=1'b0;

            // ---- latch handshake: accept a new triangle on (start && rdy). Does NOT
            //      clear the delay lines - the previous triangle's planes keep
            //      draining and its done still fires from its last_sr marker. ----
            if (start && rdy) begin
                ...
                nextp<=4'd0; cnt<=6'd1; front<=1'b1;
            end else begin
            ...
            if (inject) begin
                ...
                inj_uv_q <= inj_uv;
                guard    <= 4'd8;
                if (inj_last) front <= 1'b0;    // rdy re-asserts 8 cycles later
            ...
            // ---- emit (A3 result at d[20]); done = 1 cycle after the last plane ----
            if (d[20]) begin
                plane_valid <= 1'b1;
                plane_idx   <= idx_sr[19];
                o_ddx <= dxy_sr[6];
